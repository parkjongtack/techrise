...
if actionCate="DEL" or actionCate="RESTORE" then
    idx             = base64Decode(request.QueryString("idx"))
		...
		if actionCate="DEL" then
		   ' Dim strQuery
'				strQuery = "select fileName, thumbNail, viewNail from TB_gallery where idx=" & idx
'				call dbConOpen()
...
'				end if
'				call dbConClose()
        sql="update TB_gallery  set actResult=1  where idx = '"&idx&"' "

				
				
    else
        sql="update TB_gallery  set actResult=0  where idx = '"&idx&"' "		
    end if
end if
