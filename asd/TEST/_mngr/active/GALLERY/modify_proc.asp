...
Dim saveFileName, thumbNailFile, saveSql, returnUrl,thumbViewFile
    if Err.Number = 0 then
        if not (form.isfile) then   ' 폼에서 넘어온 값이 파일 or Text인지 체크
            saveSql = "update TB_gallery " &_
                      " set subject='"&subject&"', subjectKr='"&subjectKr&"', content='"&content&"', cate='"&cate&"', view_date='"&view_date&"',b_langCheck='"&b_langCheck&"' " &_
                      " where idx=" & idx
        else
            ...
            if not isEmptyStr(saveFileName) then  ' 업로드되는 파일이 있는 경우 기존 파일 삭제
                ...
                call dbConOpen()
                getFileName = dbCon.Execute("select fileName, thumbNail,viewNail from TB_gallery where idx="&idx)
            ...
            saveSql = "update TB_gallery " &_
                      ...
                      " modiDate=getDate(), modiIp='"&userIp&"', modiCount=modiCount+1, cate='"&cate&"', view_date='"&view_date&"',b_langCheck='"&b_langCheck&"' " &_
                      " where idx=" & idx
