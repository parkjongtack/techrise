...
<table  border="0" cellpadding="0" cellspacing="5" >
  <tr>
    ...
    <td>
		<%if sMenu="00" then%>
      <a href="javascript:goIframe()"  style='text-decoration: none;'><div style='width:100px;cursor:pointer;padding:5px; border:0px solid #ccc; font-size:13px;color:#fff; background:#3399ff; text-align:center;' "><strong>새로 업로드</strong></div></a>
		<%end if%>
    </td>
  </tr>
...
<!--
function goIframe(){
    document.getElementById("writeDiv").src="/_mngr/active/BOARD/write.asp?boardCate=<%=boardCate%>";
}
          ...
          <table border="0" cellspacing="1" cellpadding="3" class="table_3">
              ...
              <tr>
                ...
                <th style="padding:5px 20px;">
								<select name="sch_field" onkeypress = "enter(event)" <%if isEmptyStr(sch_field) then%>class="input"<%else%>class="inputins"<%end if%> style='height:23px;font-size:13px;'>
								<option value=""> ===========</option>
								<option value="A" <%if sch_field="A" then%>selected<%end if%>>Subject</option>
								<option value="B" <%if sch_field="B" then%>selected<%end if%>>Content</option>
								</select>
								</th>
